...
extern          int  Init        (unsigned long adr,   // Initialize Flash
                                  unsigned long clk,
                                  unsigned long fnc)__attribute__((section("sec_ram")));
extern          int  UnInit      (unsigned long fnc)__attribute__((section("sec_ram")));  // De-initialize Flash
extern          int  BlankCheck  (unsigned long adr,   // Blank Check
                                  unsigned long sz,
                                  unsigned char pat)__attribute__((section("sec_ram")));
extern          int  EraseChip   (void);               // Erase complete Device
extern          int  EraseSector (unsigned long adr)__attribute__((section("sec_ram")));  // Erase Sector Function
extern          int  ProgramPage (unsigned long adr,   // Program Page Function
                                  unsigned long sz,
                                  unsigned char *buf)__attribute__((section("sec_ram")));
