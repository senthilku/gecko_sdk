...
#include "rsi_board.h"
#include "rsi_chip.h"
#include "system_RS1xxxx.h" //senthil
...
uint32_t RSI_FLASH_Initialize(void)
{
  ...
  //Genrate_sample_data(); //senthil - Need to Check
...
uint32_t RSI_FLASH_Initialize(void)
{
	...
	*(uint32_t *)(TA_RESET_ADDR) = 0x1; //put TA in reset
	...
	/*Set GPIO pin MUX for QSPI*/
	RSI_EGPIO_SetPinMux(EGPIO, QSPI_PINS_PORT, M4SS_QSPI_CLK, QSPI_MODE);
	RSI_EGPIO_SetPinMux(EGPIO, QSPI_PINS_PORT, M4SS_QSPI_CSN0 , QSPI_MODE);
	RSI_EGPIO_SetPinMux(EGPIO, QSPI_PINS_PORT, M4SS_QSPI_D0 , QSPI_MODE);
	RSI_EGPIO_SetPinMux(EGPIO, QSPI_PINS_PORT, M4SS_QSPI_D1 , QSPI_MODE);
	RSI_EGPIO_SetPinMux(EGPIO, QSPI_PINS_PORT, M4SS_QSPI_D2 , QSPI_MODE);
	RSI_EGPIO_SetPinMux(EGPIO, QSPI_PINS_PORT, M4SS_QSPI_D3 , QSPI_MODE); 

	/*Initialize QSPI*/
	ROMAPI_QSPI_API->qspi_spi_init((qspi_reg_t *)(M4_QSPI_BASE_ADDRESS),&spi_configs_init,1,0,0);
